import { useState } from "react"; import { ClipboardCopy, Check, Video, Users, Bot, Settings, LogIn, UserPlus, Film } from "lucide-react";
...
return ( <main className="min-h-screen bg-gradient-to-br from-gray-950 to-black text-white p-6 md:p-12 font-sans"> <nav className="flex justify-between items-center mb-8"> <h2 className="text-2xl font-bold text-indigo-400">InnChanger AI</h2> <div className="flex gap-4"> <button className="flex items-center gap-2 text-sm hover:underline"> <LogIn size={16} /> Login </button> <button className="flex items-center gap-2 text-sm hover:underline"> <UserPlus size={16} /> Register </button> </div> </nav>

<header className="text-center space-y-4">
    <h1 className="text-4xl md:text-6xl font-bold">InnChanger AI Film Studio</h1>
    <p className="text-gray-400 md:text-lg">
      Create pro-level films with AI: scripts, VFX, 3D characters, voiceovers, and more.
    </p>
    <button
      onClick={handleCopy}
  ...
  <section className="mt-12 text-center">
    <h2 className="text-2xl font-bold text-indigo-400 mb-4">Make Your First Movie</h2>
    <p className="text-gray-400 max-w-xl mx-auto">
      Upload your image, describe your scene, and let our AI do the magic—from script to final cut.
    </p>
    <button className="mt-6 bg-green-600 hover:bg-green-700 px-6 py-3 rounded-full flex items-center gap-2 shadow-md">
      <Film size={18} /> Start Creating Now
    </button>
  </section>
